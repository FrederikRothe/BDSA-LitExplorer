...
public static class SeedExtensions
{
    ...
    private static void SeedPapers(LitExploreContext context)
    {
        ...
        if (!context.Papers.Any())
        {
            ...
            new Paper(documents[1], new List<Author> {new Author("Robert Sedgewick")}, "Eluminado", 2018, 8, 5, new List<string> {"Algorithm", "Physics"}),
            new Paper(documents[2], new List<Author> {new Author("Dijkstra")}, "El Mechanico", 1994, 7, 22, new List<string> {"Algorithm", "Physics"}),
            new Paper(documents[3], new List<Author> {new Author("Kame hame")}, "McQueen", 1994, 7, 22, new List<string> {"Physics", "Maths"}),
            new Paper(documents[4], new List<Author> {new Author("Kame hame")}, "Harald Bluetooth", 1994, 7, 22, new List<string> {"Drugs", "Pathfinding"}),
            new Paper(documents[5], new List<Author> {new Author("Doc. Dofensmirtz")}, "I don't like finibs", 1873, 7, 22, new List<string> {"Drugs", "Abuse", "Weapons"}),
            new Paper(documents[6], new List<Author> {new Author("Ronaldo")}, "El Mejor", 1995, 2, 5, new List<string> {"Weapons", "Football"}),
            new Paper(documents[7], new List<Author> {new Author("Robert Sedgewick"), new Author("Major Lazor")}, "El Mechanico", 1994, 7, 22, new List<string> {"Pathfinding"}),
            new Paper(documents[8], new List<Author> {new Author("Kame hame"), new Author("Kevin Wayne")}, "Algorithms - Fourth Edition", 2011, 4, 3, new List<string> {"Java"}),
            new Paper(documents[9], new List<Author> {new Author("Edgar F. Codd")}, "A Relational Model of Data for Large Shared Data Banks", 1970, 12, 24, new List<string> {"Database", "Relational Database", "Computer Science", "Java"}),
            new Paper(documents[10], new List<Author> {new Author("David J. Barnes"), new Author("Michael Kölling")}, "Objects First with Java", 2003, 8, 7, new List<string> {"Java", "BlueJ", "Programming", "Object Oriented Programming"}),
            new Paper(documents[11], new List<Author> {new Author("Kevin Wayne"), new Author("Doc. Dofensmirtz")}, "Python Programming", 2009, 1, 27, new List<string> {"Python", "Programming", "Weapons"})
            );
            context.SaveChanges();
        }
    }
